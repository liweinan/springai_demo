...
> Spring AI `Advisor` 接口的设计目的、**接口层次与根契约**（`adviseCall` / `adviseStream`）、**同步链与流式链**、**Reactor 流式与逐块/聚合**、**责任链机制**（含 `BaseAdvisor` 与 Chain 配合、upstream Servlet/AOP/OkHttp 对照），以及在本 Demo 中的体现。

本文档说明 **Advisor 为什么存在**、**`Advisor` / `CallAdvisor` / `BaseAdvisor` 各定什么契约**、**`StreamAdvisor` 两条平行链与 Reactor 流式语义**、**逐块处理 vs 聚合**、**Advisor 与 `DefaultAroundAdvisorChain` 如何配合**、责任链在 Tomcat Filter / Spring AOP / Spring Security / OkHttp / Netty 中的对照，并串联 `ChatClient`、`spring-ai-starter-model-deepseek` 与本项目的 `PromptLoggingAdvisor`。框架行为基于 [spring-projects/spring-ai v2.0.0](https://github.com/spring-projects/spring-ai/tree/v2.0.0)。
...
- **链只认** `adviseCall` / `adviseStream`；`before` / `after` 只是 `BaseAdvisor` 提供的环绕钩子，不是链的驱动 API
- `before` / `after` 的第二个参数 `AdvisorChain advisorChain` 供**高级场景**使用（如 `chain.copy(after)` 创建子链）；本 Demo 的 `PromptLoggingAdvisor` 只观测 request/response，**不调用** chain

**流式路径**（`adviseStream` 默认实现）与同步不同：

```java
// BaseAdvisor.java（简化）
default Flux<ChatClientResponse> adviseStream(...) {
    return Mono.just(chatClientRequest)
        .map(req -> before(req, streamAdvisorChain))      // 每轮流式请求只一次
        .flatMapMany(streamAdvisorChain::nextStream)
        .map(response -> {
            if (AdvisorUtils.onFinishReason().test(response)) {
                response = after(response, streamAdvisorChain);  // 仅 finish_reason chunk
            }
            return response;
        });
}
...
- **`before`**：在 `flatMapMany(nextStream)` 之前执行一次
- **中间 chunk**：原样透传，**不**触发 `after`
- **`after`**：仅当 chunk 带非空 `finish_reason` 时执行（一轮流结束）
...
### 5.3 Advisor 与 Chain 如何配合

**驱动链的是 `adviseCall` / `adviseStream`，不是 `before` / `after`。**

`DefaultAroundAdvisorChain` 维护两个 `Deque`（`callAdvisors` / `streamAdvisors`）。每次 `nextCall()`：

```java
// DefaultAroundAdvisorChain.java（简化）
public ChatClientResponse nextCall(ChatClientRequest chatClientRequest) {
    var advisor = this.callAdvisors.pop();           // order 最小的先弹出（最外层）
    return advisor.adviseCall(chatClientRequest, this);  // 把 chain 自身传给 Advisor
}
```

每个 Advisor 收到 **同一个 chain 对象**（`this`）。若实现 `BaseAdvisor`，模板内部再调 `chain.nextCall()`，就会弹出**下一个** Advisor，直到链尾 `ChatModelCallAdvisor`——它直接 `chatModel.call()`，**不再** `nextCall`。

```mermaid
sequenceDiagram
    participant Client as ChatClient
    participant Chain as DefaultAroundAdvisorChain
    participant Outer as ToolCallingAdvisor
    participant Inner as PromptLoggingAdvisor_BaseAdvisor
    participant Model as ChatModelCallAdvisor

    Client->>Chain: nextCall(request)
    Chain->>Outer: adviseCall(req, chain)
    Note over Outer: 自定义 do-while 工具循环
    Outer->>Chain: copy(self).nextCall(req2)
    Chain->>Inner: adviseCall(req2, chain)
    Inner->>Inner: before(req2)
    Inner->>Chain: nextCall(modified)
    Chain->>Model: adviseCall → chatModel.call()
    Model-->>Chain: ChatClientResponse
    Chain-->>Inner: response
    Inner->>Inner: after(response)
    Inner-->>Outer: response
    Outer-->>Client: final response
```

**`chain.copy(after)`**：`ToolCallingAdvisor` 在 ReAct 循环里调用 `callAdvisorChain.copy(this).nextCall(...)`，创建**子链**——只包含**当前 Advisor 之后**的节点（本 Demo 中为 `PromptLoggingAdvisor` + `ChatModelCallAdvisor`），避免递归时再次进入 `ToolCallingAdvisor`。详见 [PROMPT_LOGGING_ADVISOR.md §3](./PROMPT_LOGGING_ADVISOR.md)。

| 角色 | 类 | 与 chain 的关系 |
|------|-----|----------------|
| 链驱动器 | `DefaultAroundAdvisorChain` | `pop` + 调 `adviseCall(..., this)` |
| 环绕模板 | `BaseAdvisor` | `before` → `chain.nextCall` → `after` |
| 子链 | `chain.copy(this)` | 只跑后半段 Advisor |
| 链尾 | `ChatModelCallAdvisor` | 直调 `ChatModel`，终止 `nextCall` |

### 5.4 责任链在上游项目中的对照

Spring AI Advisor 链是经典的 **Chain of Responsibility + Around Advice**。下列 upstream 项目使用同一套路，便于对照阅读。

**统一模式**：

| 项目 | 链对象 | 「继续往下」 | 终端节点 |
|------|--------|--------------|----------|
| **Spring AI Advisor** | `CallAdvisorChain` | `chain.nextCall(req)` | `ChatModelCallAdvisor` → `chatModel.call()` |
| **Servlet Filter** | `FilterChain` | `chain.doFilter(req, res)` | `servlet.service()` |
| **Spring AOP** | `MethodInvocation` | `invocation.proceed()` | `invokeJoinpoint()` → 真实方法 |
| **Spring Security** | `VirtualFilterChain` | `chain.doFilter(req, res)` | 回到容器原 `FilterChain` |
| **OkHttp** | `Interceptor.Chain` | `chain.proceed(request)` | 真正发 HTTP |
| **Netty** | `ChannelPipeline` / `ChannelHandlerContext` | `ctx.fireChannelRead(msg)` | pipeline 末端 handler |

#### 5.4.1 Servlet Filter — Tomcat `ApplicationFilterChain`

Jakarta 规范：`Filter.doFilter(request, response, chain)`；Filter 内再调 `chain.doFilter()` 才进入下一个。

- **源码**：[apache/tomcat — `ApplicationFilterChain.java`](https://github.com/apache/tomcat/blob/main/java/org/apache/catalina/core/ApplicationFilterChain.java)

```java
public void doFilter(ServletRequest request, ServletResponse response) {
    if (pos < n) {
        Filter filter = filters[pos++].getFilter();
        filter.doFilter(request, response, this);  // Filter 内再调 chain.doFilter()
        return;
    }
    servlet.service(request, response);            // 链尾
}
```

| Servlet | Spring AI Advisor |
|---------|-------------------|
| `Filter.doFilter` | `adviseCall` 整体 |
| `chain.doFilter` | `callAdvisorChain.nextCall()` |
| `servlet.service` | `ChatModelCallAdvisor` → `chatModel.call()` |

#### 5.4.2 Spring AOP — `ReflectiveMethodInvocation.proceed()`

AOP Alliance `MethodInterceptor` 收到 `MethodInvocation`，往下走就调 `proceed()`。

- **源码**：[spring-projects/spring-framework — `ReflectiveMethodInvocation.java`](https://github.com/spring-projects/spring-framework/blob/main/spring-aop/src/main/java/org/springframework/aop/framework/ReflectiveMethodInvocation.java)

```java
public Object proceed() throws Throwable {
    if (this.currentInterceptorIndex == interceptors.size() - 1) {
        return invokeJoinpoint();  // 链尾：调真实目标方法
    }
    Object interceptor = interceptors.get(++this.currentInterceptorIndex);
    return ((MethodInterceptor) interceptor).invoke(this);  // 拦截器内再调 proceed()
}
```

与 `BaseAdvisor` 结构**最接近**：

```java
// MethodInterceptor（概念上）
Object invoke(MethodInvocation inv) {
    // before
    Object result = inv.proceed();   // ≈ chain.nextCall()
    // after
    return result;
}
```

#### 5.4.3 Spring Security — `VirtualFilterChain`

在 Servlet Filter 之上再包一层 security filter 链。

- **源码**：[spring-projects/spring-security — `FilterChainProxy.java`](https://github.com/spring-projects/spring-security/blob/main/web/src/main/java/org/springframework/security/web/FilterChainProxy.java)（内部类 `VirtualFilterChain`）

```java
public void doFilter(ServletRequest request, ServletResponse response) {
    if (this.currentPosition == this.size) {
        this.originalChain.doFilter(request, response);  // 安全链走完，回容器链
        return;
    }
...
与 `DefaultAroundAdvisorChain.nextCall()`（`pop` 一个 advisor，调 `adviseCall(..., this)`）是同一套路。

#### 5.4.4 OkHttp — `Interceptor.Chain.proceed()`

HTTP 客户端拦截器链：改 request/response 后 `chain.proceed()` 交给下一个。

- **源码**：[square/okhttp — `Interceptor.kt`](https://github.com/square/okhttp/blob/master/okhttp/src/main/kotlin/okhttp3/Interceptor.kt)

```kotlin
interface Interceptor {
    fun intercept(chain: Chain): Response
    interface Chain {
        fun proceed(request: Request): Response  // 继续链
    }
}
```

典型写法与 `PromptLoggingAdvisor` 几乎一一对应：`proceed` 前打请求日志，`proceed` 后打响应日志。

#### 5.4.5 Netty — `ChannelPipeline`

网络 I/O 事件沿 pipeline 传递：`ctx.fireChannelRead(msg)` 传给下一个 handler。Reactor Netty（Spring `WebClient` 底层传输）读到的字节沿此链上行。

- **源码**：[netty/netty — `ChannelPipeline.java`](https://github.com/netty/netty/blob/main/transport/src/main/java/io/netty/channel/ChannelPipeline.java)
...
```mermaid
flowchart LR
    subgraph patterns [Chain_of_Responsibility_upstream]
        SA[SpringAI_nextCall]
        SF[Servlet_doFilter]
        AOP[AOP_proceed]
        SS[Security_VirtualFilterChain]
        OK[OkHttp_proceed]
        NT[Netty_fireChannelRead]
    end

    SA --> Core[Current_node_logic]
    SF --> Core
    AOP --> Core
    SS --> Core
    OK --> Core
    ...
    Core --> Next[Delegate_to_next_or_terminal]
```

**阅读顺序建议**：Tomcat `ApplicationFilterChain`（最短）→ Spring `ReflectiveMethodInvocation.proceed()` → Spring AI `BaseAdvisor` → `ToolCallingAdvisor.copy(this)` 子链。
...
详细对照与 upstream 源码链接见 **§5.4**。

| Spring 生态 / 上游 | Spring AI |
|--------------------|-----------|
| Servlet `Filter.doFilter` + `chain.doFilter` | `CallAdvisor.adviseCall` + `chain.nextCall` |
| Spring AOP `MethodInterceptor` + `proceed()` | `BaseAdvisor`：`before` → `nextCall` → `after` |
| Spring Security `VirtualFilterChain` | `DefaultAroundAdvisorChain.nextCall` |
| OkHttp `Interceptor` + `chain.proceed()` | 同环绕模式；本 Demo 的 `PromptLoggingAdvisor` 类似日志拦截器 |
| `RestClient` / `WebClient` | `ChatClient` |
| 具体 HTTP 驱动（Netty pipeline） | `DeepSeekChatModel` → Reactor Netty |

可以把它理解成：Spring Web 里可以写 `HandlerInterceptor` 的 `preHandle`/`postHandle`，也可以直接实现 `Filter.doFilter`——**Spring AI 只要求你参与链（实现 `adviseCall`），不强制你用 `before`/`after` 写法**。
...
| 项目 | 源码 |
|------|------|
| Tomcat `ApplicationFilterChain` | [ApplicationFilterChain.java](https://github.com/apache/tomcat/blob/main/java/org/apache/catalina/core/ApplicationFilterChain.java) |
| Spring AOP `ReflectiveMethodInvocation` | [ReflectiveMethodInvocation.java](https://github.com/spring-projects/spring-framework/blob/main/spring-aop/src/main/java/org/springframework/aop/framework/ReflectiveMethodInvocation.java) |
| AOP Alliance `MethodInterceptor` | [MethodInterceptor.java](https://github.com/aopalliance/aopalliance/blob/master/src/main/java/org/aopalliance/intercept/MethodInterceptor.java) |
| Spring Security `FilterChainProxy` | [FilterChainProxy.java](https://github.com/spring-projects/spring-security/blob/main/web/src/main/java/org/springframework/security/web/FilterChainProxy.java) |
| OkHttp `Interceptor` | [Interceptor.kt](https://github.com/square/okhttp/blob/master/okhttp/src/main/kotlin/okhttp3/Interceptor.kt) |
| Netty `ChannelPipeline` | [ChannelPipeline.java](https://github.com/netty/netty/blob/main/transport/src/main/java/io/netty/channel/ChannelPipeline.java) |
