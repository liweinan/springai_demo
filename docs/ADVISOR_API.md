...
> Spring AI `Advisor` 接口的设计目的、**接口层次与根契约**（`adviseCall` / `adviseStream`）、责任链机制，以及在本 Demo 中的体现。

本文档说明 **Advisor 为什么存在**、**`Advisor` / `CallAdvisor` / `BaseAdvisor` 各定什么契约**、**`before`/`after` 是否必须**、责任链如何运转，并串联 `ChatClient`、`spring-ai-starter-model-deepseek` 与本项目的 `PromptLoggingAdvisor`。框架行为基于 [spring-projects/spring-ai v2.0.0](https://github.com/spring-projects/spring-ai/tree/v2.0.0)。
